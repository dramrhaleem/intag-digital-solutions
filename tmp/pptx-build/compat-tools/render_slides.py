...
def _render_presentation_with_artifact_tool(
    ...
    if proc.returncode != 0:
        # artifact-tool@2.8.31 on this Windows runtime can terminate with
        # 0xC0000409 during native canvas teardown *after* it has completed
        # every render and printed the valid JSON payload. Accept only that
        # fully materialized output; all other failures still raise.
        try:
            payload = json.loads(proc.stdout)
            rendered_paths = payload.get("paths", [])
            if rendered_paths and all(exists(path) for path in rendered_paths):
                return cast(Sequence[str], rendered_paths)
