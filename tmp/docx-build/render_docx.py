...
def _prepend_bundled_runtime_bin() -> None:
    ...
    if os.path.basename(python_root) == "python":
        ...
        if os.path.basename(dependencies_root) == "dependencies":
            ...
            runtime_bins.append(os.path.join(dependencies_root, "native", "poppler", "Library", "bin"))
    ...
    for runtime_bin in reversed(runtime_bins):
...
def convert_to_pdf(
    ...
    # Task-local Windows adapter: the bundled runtime on this workstation does
    # not include LibreOffice, but Microsoft Word is available. Keep the skill's
    # canonical rasterization path and replace only the DOCX->PDF conversion
    # step with Word's native fixed-format exporter.
    if os.name == "nt" and shutil.which("soffice") is None:
        word_exporter = join(os.path.dirname(__file__), "word_to_pdf.ps1")
        cmd_word = [
            "powershell.exe",
            "-NoProfile",
            "-ExecutionPolicy",
        ...
        proc = _run_cmd(cmd_word, env=env, verbose=verbose)
        _log_result("DOCX->PDF (Microsoft Word adapter)", cmd_word, proc)
